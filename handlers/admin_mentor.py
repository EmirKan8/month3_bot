...
from sql_tablet.bot_dp import sql_command_all, sql_command_delete, sql_command_all_ids
...
async def complete_delete(callback: types.CallbackQuery):
    ...
    await callback.answer("Удалено успешно с БД!", show_alert=True)
    await callback.message.delete()


async def distribution(message: types.Message):
    if message.from_user.id not in admins:
        await message.answer("Я не слушаю тебя!!")
    else:
